...
function CreateSectionDetails(
  ...
  return [
    {
      margin: [0, 10, 0, 10],
      text: `Fecha: ${date.toLocaleString()}`,
      bold: true,
      alignment: 'right',
    },
      ...
      table: {
        ...
        body: [
          [
            ...
            {
              text: `${
                funcionario ? `${funcionario.nombre} ${funcionario.paterno} ${funcionario.materno}` : 'SIN FUNCIONARIO'
              }`.toUpperCase(),
            },
          ],
          [
      ...
      table: {
        ...
        body: [
          ...
          [
            ...
            {
              text: `${inbox.filter((element) => element.status === statusMail.Received).length}`,
            },
            {
              text: `${inbox.filter((element) => element.status === statusMail.Pending).length}`,
            },
...
function CreateSectionList(data: communicationResponse[], date: Date): Content[] {
  ...
  if (data.length === 0) tableData.body.push([{ text: 'Sin tramites en bandeja', colSpan: 6, fontSize: 18 }]);
  return [
